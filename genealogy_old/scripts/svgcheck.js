...
// alert('Agt: '+agt+' V_maj: '+v_maj+' V_min: '+v_min);
...
is_opera=((agt.indexOf('opera')!=-1)&&(v_maj>=8));
...
var svgInstallBase="http://www.genopro.com/help/report-generator/svg/";
var svgInstallPage=svgInstallBase+"";
...
//	setCookie('SVGCheck','0',expires,'/') commented out by Ron
	}
}
...
if(window.svgInstalled)
	return;
getBrowser();
if (is_opera || is_chrome || is_mozff1_5 || is_safari || is_ie9up) {
...
if(confirm('You need a SVG viewer to display this family tree and the Adobe SVG Viewer is not installed. To install it click "OK" and follow the instructions. You can then return to this page to display the family tree. Do you wish to install the Adobe SVG Viewer now?')) {
	location=getSVGInstallPage();
	winref = window.open(getSVGInstallPage());
	}
}
...
function emitSVG(node, embedAttrs) {
if(window.svgInstalled)
	node.innerHTML='<embed '+embedAttrs+'>';
else if(window.askForSVGViewer){
	if(navigator.appName=='Netscape') {
		node.innerHTML='<p>To view this page you need an SVG viewer.<a href="'+getSVGInstallPage()+'">Click here</a> for more information.</p>';
		}
	else
		//node.innerHTML='<embed '+embedAttrs+' pluginspage="'+getSVGInstallPage()+'">';
		node.innerHTML='<embed '+embedAttrs+'>';
	}
else if(window.svgViewerAvailable)
	//node.innerHTML='<embed '+embedAttrs+' pluginspage="'+svgDownloadPage+'">';
	node.innerHTML='<embed '+embedAttrs+'>';
else {
	node.innerHTML='<p>To view this page you need an SVG viewer. There is currently no Adobe SVG ' +
	                      'Viewer available for your browser. ' +
	                      '<a href="'+svgInfoPage+'">Click here</a> for more information.</p>';
	}
}
